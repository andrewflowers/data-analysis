# Updated Baltimore arrest analysis
# 12/16/15
...
# Arrests
arrests <- read_csv("BPD_Arrests_121615.csv")
arrests$ArrestDate <- mdy(arrests$ArrestDate)
...
arrest_plot <- ggplot(data=arrests_byWeek, aes(x=Week, y=total_arrests))+
  ...
  Five38Thm +
  ggtitle("Weekly Arrests by Baltimore Police Department\nFrom 1/6/2013 through 12/5/2015")

print(arrest_plot)

#Crime

crime <- read_csv("BPD_Part_1_Victim_Based_Crime_Data_121615.csv")
crime$CrimeDate <- mdy(crime$CrimeDate)
...
crime_plot <- ggplot(data=crime_byWeek, aes(x=Week, y=total_crimes))+
  ...
  ggtitle("Baltimore, Total Crimes by week\nFrom 1/3/2010 through 12/5/2015")
...
homicide_plot<-ggplot(data=homicide_byWeek, aes(x=Week, y=total_crimes))+
  ...
  ggtitle("Total Weekly Homicides in Baltimore\nFrom 1/3/2010 through 12/5/2015")
