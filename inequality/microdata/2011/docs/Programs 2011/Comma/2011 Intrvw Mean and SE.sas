  ...
  /* 09/20/12   ADAM REICHENBERGER REPLACE PROC IMPORT TO CORRECT FORMATS    */
  /*                                                                         */
  ...
  /*  ISTUB2011.TXT -- C:\2011_CEX\Programs                                  */
  /*                                                                         */
  /***************************************************************************/
...
DATA STUBFILE (KEEP= COUNT TYPE LEVEL TITLE UCC SURVEY GROUP LINE);
  INFILE "C:\&YEAR._CEX\Programs\ISTUB&YEAR..TXT"
  PAD MISSOVER;
...
run;
DATA FMLY;
  SET FMLYI&YR1.1 (KEEP = NEWID INCLASS WTREP01-WTREP44 FINLWT21 QINTRVYR QINTRVMO) 
      FMLYI&YR1.2 (KEEP = NEWID INCLASS WTREP01-WTREP44 FINLWT21 QINTRVYR QINTRVMO) 
      FMLYI&YR1.3 (KEEP = NEWID INCLASS WTREP01-WTREP44 FINLWT21 QINTRVYR QINTRVMO) 
      FMLYI&YR1.4 (KEEP = NEWID INCLASS WTREP01-WTREP44 FINLWT21 QINTRVYR QINTRVMO)
      FMLYI&YR2.1 (KEEP = NEWID INCLASS WTREP01-WTREP44 FINLWT21 QINTRVYR QINTRVMO);

    IF (QINTRVYR=&YEAR) AND (QINTRVMO <= '03') THEN MO_SCOPE = (QINTRVMO - 1);
    ELSE IF (QINTRVYR>&YEAR) THEN MO_SCOPE = (4 - QINTRVMO);
    ELSE MO_SCOPE = 3;
...
data WORK.MTABI121                                ;
         ...
         infile 'C:\2011_CEX\Intrvw11\MTBI121.CSV' delimiter = ',' MISSOVER DSD lrecl=32767 firstobs=2 ;
          ...
          run;
DATA MTAB;
  SET MTABI&YR1.1 (KEEP = NEWID UCC COST REF_YR)
      MTABI&YR1.2 (KEEP = NEWID UCC COST REF_YR)
	  MTABI&YR1.3 (KEEP = NEWID UCC COST REF_YR)
	  MTABI&YR1.4 (KEEP = NEWID UCC COST REF_YR)
      MTABI&YR2.1 (KEEP = NEWID UCC COST REF_YR);
  IF REF_YR = &YEAR;
RUN;
...
DATA ITAB (RENAME=(VALUE=COST));
  SET ITABI&YR1.1 (KEEP = NEWID UCC VALUE REFYR)
      ITABI&YR1.2 (KEEP = NEWID UCC VALUE REFYR)
	  ITABI&YR1.3 (KEEP = NEWID UCC VALUE REFYR)
	  ITABI&YR1.4 (KEEP = NEWID UCC VALUE REFYR)
      ITABI&YR2.1 (KEEP = NEWID UCC VALUE REFYR);
  IF REFYR = &YEAR;
RUN;

DATA EXPEND (KEEP=NEWID UCC REFYR COST);
  SET MTAB ITAB;
